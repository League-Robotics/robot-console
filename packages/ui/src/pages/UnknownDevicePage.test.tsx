 ...
 * device page (ticket 008 / SUC-002, SUC-003, SUC-004).
 *
 * Three groups, matching the ticket's testing note:
 *  - Release-flash gating, ported verbatim from `DevicesTab.test.tsx`'s
 *    `DeviceCard` suite (this page's controls are moved, not
 *    redesigned).
...
import { UnknownDevicePage, MAX_LOCAL_HEX_BYTES } from "./UnknownDevicePage";
 ...
 * only state that renders flash controls (mirrors
 * `DevicesTab.test.tsx`'s own helper of the same name). */
...
/** Expected-value helper mirroring `UnknownDevicePage.tsx`'s own
...
describe("UnknownDevicePage release-flash gating", () => {
  it("shows no flash controls for an identified device", () => {
  ...
  it("shows both flash buttons for a failed-identify device", () => {
    const { el } = mountUnknownPage(failedIdentifyDevice(), { firmwareStatus: firmwareStatusFixture() });
  ...
  it("flips the robot button to enabled with no code change when availability flips", () => {
    const { el } = mountUnknownPage(failedIdentifyDevice(), {
      firmwareStatus: firmwareStatusFixture({
        robot: {
          configured: true,
          repoUrl: "https://github.com/League-Robotics/pxt-nezha-diffdrive",
  ...
  it("sends a well-formed flash-start message when Flash relay firmware is clicked", () => {
    const { el, socket } = mountUnknownPage(failedIdentifyDevice(), { firmwareStatus: firmwareStatusFixture() });
...
describe("UnknownDevicePage local-hex flow", () => {
...
describe("UnknownDevicePage post-flash navigation", () => {
  it("navigates to / when flash-result arrives ok for this endpoint", () => {
    const { el, socket } = mountUnknownPage(failedIdentifyDevice());
    act(() => {
      socket().emitMessage({
        type: "flash-result",
        endpointId: "usb-SERIAL-UNRESPONSIVE",
