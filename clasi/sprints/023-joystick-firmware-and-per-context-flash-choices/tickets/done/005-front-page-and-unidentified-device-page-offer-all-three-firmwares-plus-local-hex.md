---
id: '005'
title: Front page and unidentified device page offer all three firmwares plus local
  hex
status: done
use-cases:
- SUC-001
...
- [x] All three call sites listed above pass
      `allowedFirmware={ALL_FLASHABLE_FIRMWARE}` and
      `allowLocalHex={true}`.
- [x] Front page: an unassigned board's Flash dialog shows Flash relay
      firmware, Flash robot firmware, Flash joystick firmware, and Flash
      a hex file from disk — in that order.
- [x] Front page: an identified device's card Flash dialog shows the
...
- [x] `UnknownDevicePage`'s Flash dialog shows the same four options.
- [x] `UnknownDevicePage.test.tsx`'s existing assertions (lines ~163-164,
      currently checking only for "Flash relay firmware"/"Flash robot
      firmware") are updated to also assert "Flash joystick firmware"
      is present.
- [x] Any `FrontPage.test.tsx` assertions about the flash dialog's
