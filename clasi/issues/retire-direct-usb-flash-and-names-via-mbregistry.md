...
**Flashing moved into Sprint 018** (stakeholder decision, 2026-09-24, ticket `018-005`): `send_hex`/`flash` via the owning host's remote port is delivered by Sprint 018 rather than here, so `main` never loses the ability to flash a board once Sprint 018 disables `usbWatcher`. The dapjs/MSD flashing path itself is retired here (item 3) once Sprint 018's replacement has landed.

## Work in robot-console

1. ~~Flashing via `send_hex` + `flash` on the owning host's remote port~~ — delivered by Sprint 018, ticket `018-005`.
2. **Names** via `names_get` / `names_set` on the local socket (no create-on-read). Replaces `mbrelayRegistry.ts`.
3. **Delete** `usbWatcher`, `swdName`, the serialport / node-hid / dapjs paths, `mbrelayRegistry.ts`, and the `_mbserial` / `_mbrelay` / `_mbflash` branches of `mdnsWatcher` (WiFi `_robotlink._tcp` stays). The dapjs/MSD flashing path is part of this deletion, safe to remove once Sprint 018's mbregistry-based flashing (`018-005`) has landed.
...
- Flashing a local and a remote board works through mbregistry — delivered by Sprint 018 (`018-005`); this issue's own acceptance is that removing the dapjs/MSD code causes no regression.
