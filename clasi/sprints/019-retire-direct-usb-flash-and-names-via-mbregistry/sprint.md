...
status: roadmap
branch: sprint/019-retire-direct-usb-flash-and-names-via-mbregistry
use-cases: []
issues:
...
# Sprint 019: Retire direct USB: flash and names via mbregistry
...
replacement (`send_hex`/`flash`) was moved forward into Sprint 018,
ticket `018-005` (stakeholder decision, 2026-09-24), so this sprint's
...
§6 items 4-5. Depends on Sprint 018 (`mbregistryClient`, watcher, stream
adapter, and now also mbregistry-based flashing) landing first.

## Problem

After Sprint 018, robot-console can discover, lock, stream and flash
...
alongside its Sprint-018 replacement. The old direct-USB and
...
- Delete `usbWatcher`, `swdName`, the serialport/node-hid/dapjs paths
  ...
  replacement, `018-005`, already ships in Sprint 018), `mbrelayRegistry.ts`,
...
- Flashing a local and a remote board continues to work through
  mbregistry (delivered by Sprint 018, `018-005`) with the dapjs/MSD
...
- Anything already delivered by Sprint 018 (client, watcher, stream
  adapter, link preference order, configurable console port, and —
  moved forward by stakeholder decision, 2026-09-24 — flashing via
  `send_hex`/`flash`, ticket `018-005`). This sprint only deletes the
...
- Sprint 018 must be complete and merged first (the client, watcher,
...
- Same minimum-mbregistry-version pin as Sprint 018.
