...
describe("locked-device (mbregistry) reason rendering on the front-page card", () => {
  it("renders 'in use by <label>' verbatim on the link row", () => {
    const el = mount(
      withRouter(
        <DevicesList
          status="open"
          devices={[device(1, { links: [link("usb-1", { state: "failed", reason: "in use by alice-laptop" })] })]}
          unassigned={[]}
        />,
      ),
    );
    const row = el.querySelector('[data-testid="device-link-usb-1"]');
  ...
  it("renders the plain 'in use' reason with no label -- no undefined/null leaks into the card", () => {
    const el = mount(
      withRouter(
        <DevicesList devices={[device(1, { links: [link("usb-1", { state: "failed", reason: "in use" })] })]} unassigned={[]} status="open" />,
      ),
    );
    const row = el.querySelector('[data-testid="device-link-usb-1"]');
  ...
  it("renders the stale-lock unlock --force hint verbatim, and no control anywhere calls it", () => {
    ...
    const el = mount(
      withRouter(
        <DevicesList
          devices={[device(1, { name: "zeguz", links: [link("usb-1", { state: "unresponsive", reason })] })]}
          unassigned={[]}
          status="open"
        />,
      ),
