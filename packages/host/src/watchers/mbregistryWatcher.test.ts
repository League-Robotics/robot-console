...
describe("startMbregistryWatcher", () => {
  ...
  // Bench fix 010 -- the same bench pass found `vutev`, a JOYSTICK-
  // firmware device (no commonName/role `classifyBanner` recognizes),
  // stored with `devices.kind = "robot"`.
  describe("bench fix 010: unrecognized banner is not labeled a robot", () => {
    function joystickDevice(overrides: Partial<RegistryDevice> = {}): RegistryDevice {
      return registryDevice({
        uid: "usb:vutev",
        role: "JOYSTICK",
        common_name: null,
        ...
        raw_announcement: "device JOYSTICK vutev 12345",
        state: "connected",
        ...overrides,
      });
    }
    ...
    it("a list() entry with an unrecognized banner is not upserted as devices.kind = robot -- no devices row at all", async () => {
      const store = freshStore();
      const { client } = fakeClient([joystickDevice()]);
      const handle = startWatcher(store, client);
