...
describe("readDockState", () => {
  it("defaults to collapsed at the default height when nothing is stored", () => {
    expect(readDockState()).toEqual({ open: false, commands: false, heightPx: DEFAULT_DOCK_HEIGHT_PX });
  });
  ...
  it("returns a validly-stored open:true as-is -- collapsed is only the fallback, not a forced reset", () => {
    ...
    expect(readDockState()).toEqual({ open: true, commands: false, heightPx: 400 });
  });
  ...
  it("substitutes only the missing/invalid field, keeping the other stored one", () => {
    ...
    expect(readDockState()).toEqual({ open: true, commands: false, heightPx: DEFAULT_DOCK_HEIGHT_PX });

    window.localStorage.setItem(STORAGE_KEY, JSON.stringify({ heightPx: 250 }));
    expect(readDockState()).toEqual({ open: false, commands: false, heightPx: 250 });
  });
...
describe("writeDockState", () => {
  it("round-trips through readDockState", () => {
    writeDockState({ open: true, commands: true, heightPx: 275 });
    expect(readDockState()).toEqual({ open: true, commands: true, heightPx: 275 });
  });
  ...
  it("is a no-op that does not throw when localStorage.setItem fails", () => {
    ...
    try {
      expect(() => writeDockState({ open: true, commands: false, heightPx: 300 })).not.toThrow();
    } finally {
