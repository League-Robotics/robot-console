...
# Sprint 024: Connect to mbregistry: client, watcher and stream transport
...
(moved forward from Sprint 025 — see Solution); that issue's remaining
scope (names, deletions, `board_owner` shrink) stays with Sprint 025.
...
originally Sprint 025 scope, flashing via `send_hex`/`flash` on the
...
the gap between this sprint and Sprint 025. Names migration
(`names_get`/`names_set`) and deleting the old direct-USB paths remain
out of scope here — see Sprint 025.
...
- Flashing via `send_hex`/`flash` on the owning instance's remote TCP
  ...
  with this console's own session lock (moved forward from Sprint 025 —
...
- Radio names via `names_get`/`names_set` (Sprint 025).
- Deleting `usbWatcher`, `swdName`, the serialport/node-hid/dapjs paths,
  and `mbrelayRegistry.ts` (Sprint 025 — this sprint adds the new paths
  alongside the old ones without removing them).
- `board_owner`/`relay_leases` shrink-to-arbitration (Sprint 025).
- Hardware acceptance with two contending clients (covered by Sprint
  025's acceptance, which exercises the full replacement).
...
6. **Flash a board through mbregistry** (moved forward from Sprint 025 —
...
option / early return), not deleted — Sprint 025 deletes them. No
...
024→025 gap where such a board is undiscoverable-but-unflashable.
...
- *Context*: Sprint 025 is explicitly scoped to delete this code;
  Sprint 024's own Out-of-Scope list says so.
- *Alternatives considered*: delete now and let 025 be pure
  flash/names work.
- *Why this choice*: keeping the old code present but unreached lets a
  ...
  code revert across two sprints. It also means 024's own tests for the
...
- *Consequences*: two discovery implementations coexist in the tree
  until 025; `runtime.ts`'s composition root is the one place carrying
...
2. **Interim flashing limitation — RESOLVED (2026-09-24)**: the
   ...
   mbregistry unflashable during the 024→025 gap. See Solution, Step 2
...
3. **`relayBridger`'s DAPLink-HID reset branch's future**: this sprint
   ...
   DAPLink-HID one (Design Rationale above); Sprint 025 is expected to
   ...
   025's planning doesn't have to rediscover this dependency.
...
`connect/flasher.ts`, sprint 017); moved forward from Sprint 025 —
