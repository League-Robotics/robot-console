...
via `emitLine()`, ending in `CALA:apply
diffDrive.setConfigValue(ConfigField.RotationalSlip, <value>)`.
...
re-verification CCW) rather than in its underlying mechanics.
...
- `RotationCalibrationWizard.test.tsx`: fake link/log fixtures covering
  every acceptance criterion, including a fixture log carrying all four
  pass-stage markers plus a final `apply` line, and a separate fixture
  ending in a `fail` line at various stages (e.g. failing during the
  re-verification pass, not just the first).
